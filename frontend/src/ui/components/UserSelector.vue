...
const setCurrentUser = inject(SET_CURRENT_USER, (user: User|null) => {});
  ...
  <div class="user-selector">
    ...
    <teleport to="#modals">
      <Dropdown v-if="open" @dismiss="open = false" :top="dropdownTop()" :right="dropdownRight()">
        <UserSelectorItem v-for="user in users" :user="user"
                          :active="users.length > 1 && user.id === currentUser.id"
                          @click="onUserSelected(user)"/>

        <DropdownItem @click="open = false; showCreateUserModal = true">
          <template #icon>
            <AddIcon/>
            Crear nuevo
          </template>
