...
import { ReactiveCacheUserRepository } from '@/core/auth/infrastructure/repository/reactive-cache.user.repository.ts'
...
import { HttpWorkEntryRepository } from '@/core/tracking/infrastructure/repository/http.work-entry.repository.ts'
import { ReactiveCacheWorkEntryRepository } from '@/core/tracking/infrastructure/repository/reactive-cache.work-entry.repository.ts'
import { AllWorkEntriesByUserQuery } from './tracking/application/query/all-work-entries-by-user.query'
...
function initContainer(): Container {
  ...
  c.bind('Shared.Api.Client', () => new HttpApiClient())

  c.bind('Auth.User.Command.SignUp', () => new SignUpCommand(c.get('Auth.User.Repository')))

  c.bind('Auth.User.Query.All', () => new AllUsersQuery(c.get('Auth.User.Repository')))
