...
export class HttpUserRepository implements UserRepositoryInterface {
  ...
  async all(): Promise<User[]> {
    const response = await this.apiClient.request<ApiUser[]>('/auth/users', 'get')
  ...
  async create(name: string): Promise<User> {
    const response = await this.apiClient.request<ApiCreatedResponse>('/auth/sign-up', 'post', {
      name,
    })

    return new User(response.id, name, TimestampProvider.now(), TimestampProvider.now())
  }
}
